...
class Command(BaseCommand):
    ...
    def _setup_permissions(self):
        ...
        for app_label, perm_dict in PERMISSIONS.items():
            # Create or get ContentType - filter and create to avoid MultipleObjectsReturned
            try:
                content_type = ContentType.objects.filter(app_label=app_label).first()
                if not content_type:
                    content_type = ContentType.objects.create(app_label=app_label, model=app_label)
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(f'  ⚠ Error creating ContentType for {app_label}: {e}')
                )
                continue
