...
# Keep existing WhatsApp mutation
...
class TenantType(DjangoObjectType):
    class Meta:
        model = Tenant
        fields = ("id", "name", "subdomain", "schema_name", "property_type", "is_active", "paid_until", "on_trial", "created_at")
...
class CreateTenant(graphene.Mutation):
    class Arguments:
        name = graphene.String(required=True)
        property_type = graphene.String(required=True)  # hotel, pg, hostel, resort, apartment
    ...
    @admin_required
    def mutate(self, info, name, property_type, admin_username, admin_password, admin_email=""):
        import re
        property_type = property_type.lower().strip()
        valid_types = ["hotel", "pg", "hostel", "resort", "apartment"]
        if property_type not in valid_types:
            return CreateTenant(tenant=None, domain=None, message=f"Invalid property type. Choose from: {', '.join(valid_types)}")

        # Auto-generate subdomain: "Grand Palace" + "hotel" → "grand-palace-hotel"
        slug = re.sub(r"[^a-z0-9]+", "-", name.lower().strip()).strip("-")
        subdomain = f"{slug}-{property_type}"
        ...
        return CreateTenant(
            tenant=tenant,
            domain=domain_name,
            message=f"{name} ({property_type}) created at {domain_name}",
        )
